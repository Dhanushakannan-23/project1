...
import { FaEnvelope, FaPhone, FaMapMarkerAlt } from 'react-icons/fa'
 ...
 const Contact = () => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      .sendForm('service_9qa4d1u', 'template_aa7e9hi', form.current, {
        publicKey: 'lMePrXNOR-_nF6Rmt',
      })
      .then(
        () => {
          console.log('SUCCESS!');
    ...
    <div className='container mx-auto px-8 md:px-16 lg:px-24'>
      ...
      <div className='flex flex-col md:flex-row items-center md:space-x-12'>
        ...
        <form className='space-y-4'ref={form} onSubmit={sendEmail}>
          <div>
              <label htmlFor='name' className='mb-2 block'>First Name </label>
              <input type='text'
              name="first_name"
          ...
          <div>
              <label htmlFor='name' className='mb-2 block'>Last Name </label>
              <input type='text'
              name="last_name"
              className='w-full p-2 rounded bg-gray-800 border border-gray-600 focus:outline-none focus:border-green-300'
              placeholder='Enter your name'
          ...
          <button className='bg-gradient-to-r from-green-300 to to-blue-400 text-white hidden md:inline transform transition-transform duration-300 hover:scale-105 px-4 py-2 rounded-full' value="Send" type="submit">Send</button>
        </form>
        </div>
